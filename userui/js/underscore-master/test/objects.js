(function() {
  ...
  let testElement = typeof document === 'object' ? document.createElement('div') : void 0;
  ...
  QUnit.test('functions', function(assert) {
    let obj = {a: 'dash', b: _.map, c: /yo/, d: _.reduce};
  ...
  QUnit.test('pick', function(assert) {
    ...
    let callback = function(value, key, object) {
  ...
  QUnit.test('defaults', function(assert) {
    let options = {zero: 0, one: 1, empty: '', nan: NaN, nothing: null};
  ...
  QUnit.test('clone', function(assert) {
    let moe = {name: 'moe', lucky: [13, 27, 34]};
    let clone = _.clone(moe);
  ...
  QUnit.test('create', function(assert) {
    ...
    let created = _.create(Child.prototype, new Child);
  ...
  QUnit.test('tap', function(assert) {
    ...
    let interceptor = function(obj) { intercepted = obj; };
  ...
  QUnit.test('propertyOf', function(assert) {
    let stoogeRanks = _.propertyOf({curly: 2, moe: 1, larry: 3});
  ...
  QUnit.test('isMatch', function(assert) {
    let moe = {name: 'Moe Howard', hair: true};
    let curly = {name: 'Curly Howard', hair: false};
  ...
  QUnit.test('matcher', function(assert) {
    ...
    //null edge cases
    let oCon = _.matcher({constructor: Object});
  ...
  QUnit.test('mapObject', function(assert) {
    ...
    let ids = _.mapObject({length: 2, 0: {id: '1'}, 1: {id: '2'}}, function(n){
      return n.id;
    });
    assert.deepEqual(ids, {length: void 0, 0: '1', 1: '2'}, 'Check with array-like objects');

    // Passing a property name like _.pluck.
    let people = {a: {name: 'moe', age: 30}, b: {name: 'curly', age: 50}};
